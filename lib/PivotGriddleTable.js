...
var PivotGriddleTable = function (_Component) {
  ...
  _createClass(PivotGriddleTable, [{
    ...
    value: function checkTopPosition() {
      if (this._table === null || this.newTable === null) return;

      var elementScroll = this.props.elementScroll;

      var element = document.querySelector(elementScroll);
      var fixedHeader = document.querySelector('#' + FIXED_HEAD_ID);
    ...
    value: function createRows() {
      ...
      if (!getRowKey) rows = _utils2.default.array.uniqueKey(rows, 1);
      rows.forEach(function (row) {
        var grouping = groupBy && row.children;
        var baseKey = getRowKey && row[getRowKey] ? row[getRowKey] : 'row-' + row.$$keys.$$object_key;
        var key = baseKey;
        if (grouping) {
          var groupRows = [];
          ...
          var childRows = _utils2.default.array.uniqueKey(row.children.slice(1), 2);
          var totals = void 0;
