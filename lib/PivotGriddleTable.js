...
var PivotGriddleTable = function (_Component) {
  ...
  function PivotGriddleTable(props) {
    ...
    _this.onShowChild = _this.onShowChild.bind(_this);

    if (!(typeof _this.onShowChild === 'function')) {
      throw new TypeError('Value of "this.onShowChild" violates contract.\n\nExpected:\n() => any\n\nGot:\n' + _inspect(_this.onShowChild));
    }
  ...
  _createClass(PivotGriddleTable, [{
    ...
    key: 'calcRow',
    value: function calcRow(prev, curr) {
      var fixed = this.props.rowMetadata.fixedNum || 0;
      var del = Math.pow(10, fixed) || 10;
      var next = _extends({}, prev);
      if (curr === null) return next;
      if (!next.count) next.count = 0;
      if (!next.sum) next.sum = 0;
      next.count += 1;
      var oldSum = parseFloat(next.sum, 10) * del;
      var currValue = parseFloat(curr, 10) * del;
      var newSum = ((oldSum + currValue) / del).toFixed(fixed);
      next.sum = newSum;
    ...
    key: 'calculateRows',
    value: function calculateRows(row, columns) {
      ...
      columns.forEach(function (col) {
        if (col.children) {
          _this3.calculateRows(row, col.children, [].concat(_toConsumableArray(path), [col.column]), data, depth + 1);
        } else {
          var fullPath = [].concat(_toConsumableArray(path), [col.column]);
          var prevData = (0, _lodash.get)(data, fullPath, {});
          var nextData = (0, _lodash.get)(row, fullPath, null);
          (0, _lodash.set)(data, fullPath, _this3.calcRow(prevData, nextData));
        }
      });
      return data;
    }
    ...
    value: function createRows() {
      ...
      var getRowKey = rowMetadata.getRowKey;

      var renderer = [];
      rows.forEach(function (row, idx) {
        var grouping = groupBy && row.children;
        var baseKey = getRowKey && getRowKey(row) || 'row-' + (0, _objectHash2.default)({ row: row, idx: idx });
        var key = baseKey;
        if (grouping) {
          var groupRows = [];
          var renderChildRows = [];
          ...
          if (groupSettings.type === 'column') {
            ...
            groupRows.push(_this4.renderRow(firstRow, renderColumns, key, rowSpan));
            ...
            var colSpan = _utils2.default.object.getColSpan({ children: childColumns });
            groupRows.push(_this4.renderRow(_firstRow, firstRowColumns, key, _rowSpan, false, false, colSpan));
          }
          if (childRows.length >= 1) {
            totals = {};
            childRows.forEach(function (childRow) {
              key = baseKey + '-c-' + (0, _objectHash2.default)(childRow);
              renderChildRows.push(_this4.renderRow(childRow, childColumns, key, false));
              if (childColumns.length > 1) {
                totals = Object.assign(totals, _this4.calculateRows(childRow, childColumns, [], totals));
              }
            });
          ...
          if (groupSettings.type === 'row' && totals) {
            ...
            if (groupSettings.totalPosition === 'top') {
              renderChildRows.unshift(_this4.renderRow(totals, renderColumns, key, false, false, true));
            } else if (groupSettings.totalPosition === 'bottom') {
              renderChildRows.push(_this4.renderRow(totals, renderColumns, key, false, false, true));
            }
          ...
          renderer.push(_this4.renderRow(row, groupingColumns, key));
        } else {
          var rrows = _this4.renderRow(row, renderColumns, key, false);
          renderer.push(rrows);
        }
      });
      return renderer;
    ...
    value: function render() {
      ...
      var headerClassName = rowMetadata.headerCssClassName ? rowMetadata.headerCssClassName : '';
...
PivotGriddleTable.propTypes = {
  ...
  elementScroll: _propTypes2.default.oneOfType([_propTypes2.default.bool, _propTypes2.default.string]).isRequired
};
