...
var PivotGriddle = (_temp = _class = function (_Component) {
  ...
  function PivotGriddle(props) {
    ...
    var grid = Object.assign(defaultGroupSettings, props.groupSettings);
    var rowMeta = Object.assign(defaultRowMeta, props.rowMetadata);
  ...
  _createClass(PivotGriddle, [{
    ...
    value: function getRenderColumns() {
      ...
      if (groupBy && groupSettings.type === 'row') {
        renderColumns = renderColumns.filter(function (item) {
    ...
    value: function getGroupRows() {
      ...
      if (groupBy) {
        ...
        sortableRows = Object.keys(grouping).map(function (value) {
          ...
          var val = value != 'undefined' ? value : ''; // eslint-disable-line
          obj[groupBy] = val;
    ...
    value: function sortingRows(rows) {
      ...
      if (groupBy) {
        _utils2.default.array.sortDir(sortableRows, this.state.groupBySort, groupBy);
        if (sortBy && childs) {
        ...
        if (depthChildrenKey) {
          sortableRows = sortableRows.map(function (row) {
            if (row[depthChildrenKey] && row[depthChildrenKey].length > 1) {
              row[depthChildrenKey] = _this3.sortingRows(row[depthChildrenKey]);
            }
            return row;
          });
        }
    ...
    value: function onPageChange(nextPage) {
      ...
      var customPageChange = this.props.customPageChange;

      if (customPageChange && typeof customPageChange === 'function') {
        var getCustomPage = customPageChange(nextPage, this.state.pageSize);
        if (getCustomPage && getCustomPage instanceof Promise) {
          getCustomPage.then(function (payload) {
            var obj = {};
            if (_this4.props.infinityScroll) {
              obj.rows = _this4.state.rows.concat(payload.rows);
            } else {
    ...
    value: function render() {
      ...
      var _props5 = this.props,
          ...
          fixedTableHead = _props5.fixedTableHead,
          depthChildrenKey = _props5.depthChildrenKey,
          simplePagination = _props5.simplePagination;
  ...
  depthPadding: _propTypes2.default.number,
  ...
  rowMetadata: _propTypes2.default.object,
  ...
  groupSettings: _propTypes2.default.object
}, _class.defaultProps = {
  columns: [],
  hiddenColumns: [],
  rows: [],
