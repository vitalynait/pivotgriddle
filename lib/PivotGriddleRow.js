  ...
  _createClass(PivotGriddleRow, [{
      ...
      if (row.$$showChild) {
        row[depthChildrenKey].forEach(function (child) {
          ...
          if (child[depthChildrenKey] && child[depthChildrenKey].length >= 1) {
            rows.push.apply(rows, _toConsumableArray(_this7.renderManyRow(child, columns, sRowKey)));
          } else {
            rows.push(_this7.renderRow(child, columns, false, false, true, sRowKey));
          }
        });
      }
      return rows;
    ...
    value: function render() {
      var _props5 = this.props,
          ...
          rowSpan = _props5.rowSpan,
          groupBy = _props5.groupBy,
          wrapping = _props5.wrapping,
          totalRow = _props5.totalRow,
          rowKey = _props5.rowKey,
          colSpan = _props5.colSpan;
