...
var PivotGriddleRow = function (_Component) {
  ...
  _createClass(PivotGriddleRow, [{
    ...
    value: function preRenderCell(col, row, rowData, totalRow, rowSpan) {
      ...
      if (!col.children) {
        ...
        return col.children.map(function (ccol) {
          return _this2.preRenderCell(ccol, row[col.column], rowData, totalRow, rowSpan);
        });
      }
    }
  }, {
    ...
    value: function renderRow(row, columns, rowSpan) {
      ...
      return _react2.default.createElement(
        ...
        columns.map(function (col) {
          return _this3.preRenderCell(col, row, row, totalRow, rowSpan);
        })
      );
    }
  }, {
    ...
    value: function renderShowChild() {
      ...
      var element = showChild ? this.props.rowExpandedComponent : this.props.rowCollapsedComponent;
      var key = rowKey + '-showChild';
      return _react2.default.createElement(
        'td',
        { className: 'systemCell', key: key, onClick: function onClick() {
            return _this4.onChildShow();
          } },
        element
      ); //eslint-disable-line
    }
    ...
    value: function renderDepthRow(row, columns) {
      ...
      if (this.state.showChild) {
        var childs = _utils2.default.array.uniqueKey(row[depthChildrenKey], 3);
        childs.forEach(function (child) {
          rows.push(_this5.renderRow(child, columns, false, false, true, '-' + child.$$keys.$$object_key));
        });
      }
        ...
        _react2.default.createElement(
          ...
          columns.map(function (col) {
            if (!col.children && col.column !== 'showChild') {
              return _this5.renderCell(row, col, false);
            } else if (col.column === 'showChild') {
              return _this5.renderShowChild();
            } else {
              var childColumns = col.children;
              var childData = row[col.column];
              return childColumns.map(function (ccol) {
                return _this5.renderCell(childData, ccol, false, row);
              });
            }
          })
        ),
